...
- ~~**RF-5.9** (F2a) La barra de herramientas de dibujo es **flotante y arrastrable** (puede superponerse a la de timeframes) y su posición persiste entre sesiones~~ — **sustituida por RF-5.24 (F4b)**: con catorce botones la barra flotante tapaba el gráfico
...
- **RF-5.16** (F3, revisado en F4b) Las herramientas: horizontal (con y sin extensión), tendencia, rectángulo/zona, curva, arco, texto — más la zona de dos niveles (dos horizontales vinculadas que se mueven juntas; con Shift se ajusta una sola). El **punto** se retiró en F4b: no aportaba nada que no hiciera una marca de precio
- **RF-5.17** (F4) **Cambiar de timeframe conserva el rango temporal visible**, no salta al presente. Tope de 12.000 velas visibles y suelo de 20 (`cfg.tfChangeMaxBars` / `cfg.tfChangeMinBars`): fuera de esos límites se centra en el mismo instante y se enseñan las que caben, avisando en la barra de estado. Con la ventana en el pasado se cargan velas nuevas al desplazarse hacia el presente y **End** vuelve a él conservando el ancho
- **RF-5.18** (F4) Flechas **↑ / ↓** para subir y bajar de timeframe en el orden de la barra. A diferencia del click, que conserva el **tramo**, las flechas conservan el **ancho de vela**: bajar de temporalidad acerca y subir aleja, viendo más o menos tramo con las velas del mismo tamaño. Se queda quieto el borde derecho si se está pegado al presente, y el centro de la pantalla si se está mirando el pasado. Dentro de un input o un select las flechas son del control
- **RF-5.19** (F4) El timeframe y la posición se restauran al recargar. Si el borde derecho estaba pegado al presente se conserva el **ancho** de la ventana, no el instante
...
- **RF-6.4** ✅ (F5) Backup periódico por capas: estado irreemplazable (dibujos, alertas, huecos, progreso) cada 6 h, un día de `candles_1s` al día y `candles_1m` entera cada semana, a almacenamiento de objetos compatible con S3. **Con restauración verificada contra manifiesto**, no solo copiada
- **RF-6.5** (F5) **Alertas de precio server-side**: motor en proceso propio que evalúa cada vela de 1s contra los niveles activos y notifica por Telegram. Cruce al alza, a la baja o en cualquier dirección; "solo una vez" o recurrente con banda de rearme, cooldown y tope diario; historial de disparos con estado de entrega. Funciona con el ordenador del usuario apagado
...
14. **Los índices lógicos de LWC no admiten fracciones (F3)** — `timeScale().logicalToCoordinate(l)` devuelve **0** (no `null`, no un error) si `l` no es entero, y `coordinateToLogical(x)` **redondea** al índice de la vela más cercana. Usarlas tal cual cuantiza cualquier dibujo a velas enteras y, tras un cambio de timeframe, coloca en `x = 0` todo lo que no caiga justo sobre una vela — que es casi todo. Es la misma clase de trampa que el `coordinateToTime` del plugin retirado, y silenciosa igual: no hay excepción, solo una coordenada plausible y equivocada. La solución es interpolar entre dos anclas enteras, que sí resuelve bien (`logicalOfX`/`xOf` en `web/src/draw/engine.js`). Lo destapó el test de cambio de timeframe al comprobar el **efecto** (dónde se pinta) en vez de la intención (qué guarda el modelo).
15. **La posición del gráfico en LWC es relativa a la ÚLTIMA vela (F4)** — el `visibleLogicalRange` que se lee y se escribe son índices, pero por dentro la escala guarda la distancia al último dato. Al añadir velas por la derecha con `setData` la ventana **se va con ellas**: cargando el tramo siguiente de histórico, la vista saltaba de marzo de 2020 a diciembre de 2025 sin que nadie la tocara. Al añadir por la izquierda los índices se desplazan y el efecto es el mismo con otro signo. Solución: guardar el rango antes y volver a fijarlo después de cada `setData` (`cargarMasNuevas` / `cargarMasViejas` en `web/src/app.js`). Se ve solo si se comprueba **dónde acaba la vista** (con `getVisibleRange()`, en tiempo) y no solo que los datos llegaron.
16. **`TRUNCATE` no vacía una continuous aggregate (F5)** — lo materializado vive en OTRA hypertable (`_timescaledb_internal._materialized_hypertable_N`) y sobrevive al truncado de la tabla origen. Refrescar con los dos límites a `NULL` tampoco lo borra: comprobado, tras resembrar quedaba una vela de 1h de la ventana anterior, y con velas de 1d/1h más viejas que `candles_1m` los tests apuntaban a un tramo vacío. En una base de datos de usar y tirar la salida es borrar el ESQUEMA entero y volver a migrar — con una pega encadenada: al hacerlo se va la extensión `timescaledb` y el backend que la tenía cargada se niega a recrearla (*"has already been loaded with another version"*), así que hay que forzar conexiones nuevas (`pool.Reset()`) antes de migrar.
17. **`pg_dump -t` de una hypertable NO lleva los datos (F5)** — devuelve la definición de la tabla padre y nada más: 881 bytes para `candles_1m`, que tiene 3,6 millones de filas. Los datos están en los chunks, en otro esquema. Para copiar una hypertable hay que enumerar sus chunks o, como aquí, exportar con `COPY ... TO STDOUT` y restaurar con `COPY ... FROM`.
18. **`docker compose run` con la salida a `/dev/null` mata el contenedor (F5)** — a los tres segundos, sin mensaje. La siembra de la base de datos de test se iba a medias —justo después de borrar el esquema— y el guion seguía como si nada, con la base de datos vacía y comprobaciones fallando por motivos que no tenían nada que ver. Con la salida a un fichero funciona. Corolario: `docker compose up -d --build` tampoco construye los servicios que están tras un `profile`, así que el primer backup corrió con un binario de tres despliegues antes.
...
| **F5**  | ✅ HECHO — Aislamiento de los tests (base de datos propia con semilla sintética), backups por capas con restauración verificada, y alertas de precio server-side con notificación por Telegram | 12-20 |
| **F4**  | ✅ HECHO — Navegación temporal (el rango visible sobrevive al cambio de timeframe y a la recarga, flechas de teclado), estilos por defecto y plantillas, deshacer/rehacer, legend OHLC, imán, ocultar/bloquear y duplicar | 10-16 |
